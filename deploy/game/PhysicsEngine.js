 ...
 */
import { CORE_SETTINGS, StructureType, STRUCTURE_BASE_STATS, FUEL_ADJACENCY, ECONOMY, UpgradeType, UPGRADE_DEFINITIONS, EXOTIC_FUEL, getFuelHeatGeneration, } from './BalanceConfig.js';
/**
...
export class PhysicsEngine {
    constructor(gridManager, getUpgradeLevel) {
     ...
     */
    getEffectiveMeltTemp(structure) {
        const baseTemp = STRUCTURE_BASE_STATS[structure].meltTemp;
        let upgradeType = null;
        switch (structure) {
            case StructureType.FuelRod:
                upgradeType = UpgradeType.MeltTempFuelRod;
                break;
            case StructureType.Ventilator:
                upgradeType = UpgradeType.MeltTempVentilator;
                break;
            case StructureType.HeatExchanger:
                upgradeType = UpgradeType.MeltTempHeatExchanger;
                break;
            case StructureType.Insulator:
                upgradeType = UpgradeType.MeltTempInsulator;
                break;
            case StructureType.Turbine:
                upgradeType = UpgradeType.MeltTempTurbine;
                break;
            case StructureType.Substation:
                upgradeType = UpgradeType.MeltTempSubstation;
                break;
        }
        if (upgradeType) {
     ...
     */
    getEffectiveHeatDissipation(cell) {
     ...
     */
    getEffectivePowerSaleRate(cell) {
        if (cell.structure !== StructureType.Substation)
            return 0;
        const baseStats = STRUCTURE_BASE_STATS[cell.structure];
    ...
    processHeatGeneration() {
        ...
        for (let y = 0; y < gridSize; y++) {
            for (let x = 0; x < gridSize; x++) {
                ...
                // Exotic fuel scaling
                if (cell.isExotic) {
                    const tempMultiplier = Math.min(EXOTIC_FUEL.MAX_MULTIPLIER, EXOTIC_FUEL.BASE_MULTIPLIER + (cell.heat / 1000) * EXOTIC_FUEL.HEAT_SCALING);
